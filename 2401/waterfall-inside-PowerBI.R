...
waterfall_chart <- function(gdf, width=0.45, text_fmt=fmt_c1_e3, text_vert=+1, text_ang=25, left_fmt=scale_cy_c1_e3) {
    s <- 0;
    for(k in 1:nrow(gdf)) {
...
#    g <- g + geom_bar(aes(x=FY_long, y=amt, fill=fill), stat="identity", show.legend=FALSE);
   ...
   g <- g + geom_text(aes(x=(x1+x2)/2, y=ym, label=text_fmt(amt)), angle=text_ang, vjust=text_vert, show.legend=FALSE);
